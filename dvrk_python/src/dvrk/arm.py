...
#  This work is based on Anton Deguet's work
...
class arm(object):
    ...
    def __init_arm(self, arm_name, ros_namespace = '/dvrk/'):
        ...
        self.__gripper_position = numpy.array(0, dtype=numpy.float)
        ...
        rospy.Subscriber(self.__full_ros_namespace + '/gripper_position_current',
                         Float32, self.__gripper_position_cb)
    ...
    def set_wrench(self, wrench):
        ...
        if (not self.__dvrk_set_state('DVRK_EFFORT_CARTESIAN')):
            return False
        self.__set_wrench_spatial_pub.publish(wrench)
    ...
    def __gripper_position_cb(self, data):
        """Callback for gripper position.

        :param data: the current gripper position"""
        self.__gripper_position = data.data
